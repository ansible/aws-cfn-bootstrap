...
class FileTool(object):
    ...
    def apply(self, action, auth_config):
        ...
        for (filename, attribs) in sorted(action.items(), key=lambda pair: pair[0]):
            if not os.path.isabs(filename):
                raise ToolError(
                    'File specified with non-absolute path: %s' % filename)

            # The only difference between a file and a symlink is hidden in the mode
            file_is_link = "mode" in attribs and stat.S_ISLNK(
                int(attribs["mode"], 8))

            if file_is_link:
                if "content" not in attribs:
                    raise ToolError(
                        "Symbolic link specified without a destination")
            ...
            if not os.path.isdir(parent):
                if not os.path.exists(parent):
                    log.debug(
                        "Parent directory %s does not exist, creating", parent)
                    os.makedirs(parent)
                else:
                    raise ToolError(
                        "Parent directory %s exists and is a file" % parent)
            ...
            with self.backup(filename, files_changed):
                if file_is_link:
                    log.debug("%s is specified as a symbolic link to %s",
                              filename, attribs['content'])
                    os.symlink(attribs["content"], filename)
                else:
                    file_is_text = 'content' in attribs and not self._is_base64(
                        attribs)
                ...
                if "mode" in attribs:
                    log.debug("Setting mode for %s to %s",
                              filename, attribs["mode"])
                    os.chmod(filename, stat.S_IMODE(int(attribs["mode"], 8)))
                else:
                    if currentFileExists:
                        log.debug("No mode specified for %s. Setting permissions of the existing file: %s.", filename, oct(
                            currentFileMode))
                        os.chmod(filename, currentFileMode)
                    else:
                        # get attributes that the file was just created
                        currentFileMode = stat.S_IMODE(
                            os.stat(filename).st_mode)
                        # filter the curent mode by giving up others writable
                        currentFileMode = currentFileMode & 0o775
                        log.debug("No mode specified for %s. The file will be created with the mode: %s", filename, oct(
                            currentFileMode))
                        os.chmod(filename, currentFileMode)

                security.set_owner_and_group(
                    filename, attribs.get("owner"), attribs.get("group"))
    ...
    def backup(self, filename, files_changed):
        ...
        try:
            ...
            linkmode = backup_file and os.path.islink(
                backup_file) or os.path.islink(filename)
            ...
            if not backup_file or linkmode or not FileTool.compare_file_contents(backup_file, filename):
                ...
                try:
                    ...
                    log.warn("Error restoring backup file %s: %s",
                             backup_file, str(t))
    ...
    def _render_template(self, content, context):
        """Tries rendering template with chevron first. If chevron is not present then uses pystache."""
        log.debug('Rendering as Mustache template')
        try:
            import chevron
            if isinstance(content, str):
                return chevron.render(content, context)
            else:  # handle byte content
                return chevron.render(content.decode('UTF-8'), context)
        except ImportError:
            log.debug("Chevron is not installed trying pystache")
        except Exception as e:
            raise ToolError("Failed to render content as a Mustache template: %s" % getattr(e, 'message', repr(e)))

        try:
            from pystache.renderer import Renderer
            return Renderer(string_encoding='utf-8', file_encoding='utf-8').render(content, context)
        except ImportError:
            raise ToolError("Failed to render the template since pystache and chevron are missing")
        except Exception as e:
            raise ToolError("Failed to render content as a Mustache template: %s" % getattr(e, 'message', repr(e)))
    ...
    def _write_remote_file(self, source, auth, dest, context):
        ...
        remote_contents = util.EtagCheckedResponse(
            requests.get(source, **opts))

        if context is None:
            remote_contents.write_to(dest)
        else:
            dest.write(self._render_template(
                remote_contents.contents(), context).encode())
