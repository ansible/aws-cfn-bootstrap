...
from cfnbootstrap.util import retry_on_failure, timeout, Credentials
import datetime
import logging
import re
import json
...
class CloudFormationClient(aws_client.Client):
    ...
    _endpoints = {
        "us-iso-east-1": "https://cloudformation.us-iso-east-1.c2s.ic.gov",
        "us-iso-west-1": "https://cloudformation.us-iso-west-1.c2s.ic.gov",
        "us-isob-east-1": "https://cloudformation.us-isob-east-1.sc2s.sgov.gov",
    ...
    def __init__(self, credentials, url=None, region='us-east-1', proxyinfo=None):
        ...
        super(CloudFormationClient, self).__init__(
            credentials, True, endpoint, signer, proxyinfo=proxyinfo)
    ...
    def regionForEndpoint(cls, endpoint):
        match = re.match(
            r'https://cloudformation.([\w\d-]+).amazonaws.com', endpoint)
        if match:
    ...
    def describe_stack_resource(self, logicalResourceId, stackName, request_credentials=None):
        ...
        """
        log.debug("Describing resource %s in stack %s",
    ...
    def register_listener(self, stack_name, listener_id=None, request_credentials=None):
        ...
        Throws an IOError on failure.
        """
        log.debug("Registering listener %s for stack %s",
    ...
    def elect_command_leader(self, stack_name, command_name, invocation_id, listener_id=None, request_credentials=None):
        ...
        result_data = self._call(
            params, request_credentials=request_credentials).json()
    ...
    def get_listener_credentials(self, stack_name, listener_id=None, request_credentials=None):
        ...
        body = resp.json()[
            'GetListenerCredentialsResponse']['GetListenerCredentialsResult']['Credentials']
...
class Listener(object):
    ...
    def __init__(self, resp):
        result = resp.json()[
            'RegisterListenerResponse']['RegisterListenerResult']
...
class StackResourceDetail(object):
    ...
    def __init__(self, resp):
        detail = resp.json()[
            'DescribeStackResourceResponse']['DescribeStackResourceResult']['StackResourceDetail']

        self._description = detail.get('Description')
        self._lastUpdated = datetime.datetime.utcfromtimestamp(
            detail['LastUpdatedTimestamp'])
