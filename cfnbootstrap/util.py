...
def extend_backoff(durations, max_sleep=20):
    ...
    """
    durations.append(random.random() *
                     min(max_sleep, (2 ** len(durations) - 1)))
...
class EtagCheckedResponse(object):
    ...
    def __init__(self, response):
        ...
        etag = response.headers['etag'].strip('"') if 'etag' in response.headers and endpoint_tool.is_service_url(
            "AmazonS3", response.url) else None
        if response.headers.get('x-amz-server-side-encryption') == 'aws:kms':
            ...
            log.warn(
                'cannot check consistency of file uploaded multipart; etag has - character present')
            etag = None

        self._etag = etag

        # hashlib will throw ValueError for md5 on FIPS-enabled instances unless usedforsecurity is set to False
        # however we need to check if system's libcrypto library supports usedforsecurity flag
        # S3 only offers md5 checksums at present
        if self._etag:
            try:
                self._digest = hashlib.md5()
            except ValueError as e:
                if "disabled for fips" in str(e):
                    if 'usedforsecurity' in inspect.getargspec(hashlib.new).args:
                        self._digest = hashlib.new('md5', usedforsecurity=False)
                    else:
                        log.warn(
                            'libcrypto does not support usedforsecurity flag; cannot use md5 checksum on FIPS enabled instance')
    ...
    def _check_digest(self):
        ...
        if self._etag != final_digest:
            raise ChecksumError("Expected digest %s; received %s" %
                                (self._etag, final_digest))
    ...
    def _decorate(f):
        def _retry(*args, **kwargs):
            ...
            for duration in durations:
                if duration > 0:
                    log.debug(
                        "Sleeping for %f seconds before retrying", duration)
...
def _get_metadata_path(path):
    return "http://169.254.169.254/latest{}".format(path)


def _get_session_token():
    """
    Try to get token for getting instance metadata(https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-instance-metadata-service.html)
    """
    resp = requests.put(_get_metadata_path('/api/token'),
                        headers={'X-aws-ec2-metadata-token-ttl-seconds': '21600'},
                        timeout=REQUEST_TIMEOUT)
    return resp.text.strip() if resp.status_code == 200 else ''


def _get_metadata_response(path, token, _timeout=None):
    if not token:
        resp = requests.get(path, timeout=_timeout, proxies={'no_proxy': '169.254.169.254/32'})
    else:
        resp = requests.get(path, timeout=_timeout, proxies={'no_proxy': '169.254.169.254/32'},
...
def get_instance_identity_document():
    ...
    metadata_path = _get_metadata_path('/dynamic/instance-identity/document')
    resp = _get_metadata_response(metadata_path, token)
...
def get_instance_identity_signature():
    token = _get_session_token()
    metadata_path = _get_metadata_path('/dynamic/instance-identity/signature')
    resp = _get_metadata_response(metadata_path, token)
    resp.raise_for_status()
    return resp.text.rstrip()
...
def _fetch_instance_id():
    token = _get_session_token()
    metadata_path = _get_metadata_path('/meta-data/instance-id')
    request_timeout_in_seconds = 2
    resp = _get_metadata_response(metadata_path, token, _timeout=request_timeout_in_seconds)
    resp.raise_for_status()
...
def get_role_creds(name):
    ...
    metadata_path = _get_metadata_path(
        '/meta-data/iam/security-credentials/{}'.format(name))
    resp = _get_metadata_response(metadata_path, token)
    resp.raise_for_status()
    role = resp.json()
    return Credentials(role['AccessKeyId'], role['SecretAccessKey'], role['Token'],
...
def interpret_boolean(input):
    ...
    input = input.lower().strip() if isinstance(input, str) else input
...
def get_proxy_options(parser):
    proxy_group = OptionGroup(parser, "Proxy",
                              "Options for specifying proxies. Format: [scheme://][user:password@]host:port")

    proxy_group.add_option(
        "", "--http-proxy", help="A (non-SSL) HTTP proxy", type="string", dest="http_proxy")
    proxy_group.add_option(
        "", "--https-proxy", help="An HTTPS proxy", type="string", dest="https_proxy")
...
def get_cred_options(parser):
    creds_group = OptionGroup(
        parser, "AWS Credentials", "Options for specifying AWS Account Credentials.")
...
def extract_credentials(path):
    ...
    if os.name == 'posix':
        ...
        if stat.S_IRWXG & mode or stat.S_IRWXO & mode:
            raise IOError(None,
                          "Credential file cannot be accessible by group or other. Please chmod 600 the credential file.")
    ...
    if not access_key or not secret_key:
        raise IOError(
            None, "Credential file must contain the keys 'AWSAccessKeyId' and 'AWSSecretKey'")
...
class ProcessResult(object):
    ...
    @property
    def stdout(self):
        return _ensure_str(self._stdout)

    @property
    def stderr(self):
        return _ensure_str(self._stderr)
...
class ProcessHelper(object):
    ...
    def __init__(self, cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=None, cwd=None):
        ...
        if not env:
            self._env = None
        elif os.name == WINDOWS:  # stringify the environment in Windows, which cannot handle unicodes
            self._env = dict(((str(k), str(v)) for k, v in iter(env.items())))
        else:
            self._env = dict(env)
        self._cwd = cwd
    ...
    def call(self):
        ...
        if os.name == WINDOWS:
            ...
            if isinstance(self._cmd, str):
                ...
                command = subprocess.list2cmdline(self._cmd)

            # A quick test to see if I/O redirection works or not since IO redirection fails for Windows 2016
            io_redirection_works = True
            try:
                p = subprocess.Popen(
                    "echo \'hello\'",
                ...
                log.info("IO redirection does not work, the output of commands will not be written to logs", exc_info=e)
                io_redirection_works = False

            process = subprocess.Popen(
                self._cmd,
                stdout=self._stdout if io_redirection_works else subprocess.DEVNULL,
                stderr=self._stderr if io_redirection_works else subprocess.DEVNULL,
                shell=isinstance(self._cmd, str),
                env=self._env,
                cwd=self._cwd
            )

            if io_redirection_works:
                log.info("IO redirection is working. Redirecting the output to logs")
                return_data = process.communicate()
            else:
                log.info("IO redirection does not work. Redirecting the output to null")
                process.wait()
                return_data = [
                    "", ""
                ]
...
class LoggingProcessHelper(object):
    ...
    def call(self):
        cmd_log.info(60 * "=")
        cmd_log.info(_ensure_str(self.name))
            ...
            if not self._creds or self._creds.expiration - datetime.timedelta(hours=2) < datetime.datetime.utcnow():
                ...
                try:
                    ...
                    if not self._creds or self._creds.expiration < datetime.datetime.utcnow():
                        raise RuntimeError(
                            'Credentials have expired and refresh failed.')
                    log.exception(
                        'Exception while refreshing credentials; will refresh on next access')
...
def log_response(resp, *args, **kwargs):
    wire_log.debug('Response: %s %s [headers: %s]',
