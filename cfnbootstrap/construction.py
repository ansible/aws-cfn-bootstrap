...
class WorkLog(object):
    ...
    def build(self, metadata, configSets, strict_mode=False):
...
class CloudFormationCarpenter(object):
    ...
    def __init__(self, config, auth_config, strict_mode=False):
        self._config = config
        ...
        if self._config.packages:
            for manager, packages in sorted(self._config.packages.items(), key=cmp_to_key(CloudFormationCarpenter._pkgsort)):
                if manager in CloudFormationCarpenter._packageTools:
                    if manager == "rpm":
                        changes['packages'][manager] = CloudFormationCarpenter._packageTools[manager]().apply(packages, self._auth_config, self.strict_mode)
                    else:
...
class Contractor(object):
    ...
    def __init__(self, model, strict_mode=False):
