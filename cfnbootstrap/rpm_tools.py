...
class YumTool(object):
    ...
    def apply(self, action, auth_config=None):
        ...
        cache_result = LoggingProcessHelper(['yum', '-y', 'makecache'], name=u'yum makecache').call()
        ...
        for pkg_name in action:
            if action[pkg_name]:
                if isinstance(action[pkg_name], str):
    ...
    def _pkg_installed(self, pkg):
        result = LoggingProcessHelper(['yum', '-y', 'list', 'installed', pkg], name="yum list installed %s" % pkg).call()
    ...
    def _pkg_available(self, pkg):
        ...
        result = LoggingProcessHelper(['yum', '-y', '--showduplicates', 'list', 'available', pkg], name="yum list available %s" % pkg).call()

        return result.returncode == 0

class RpmTool(object):

    def apply(self, action, auth_config=None, strict_mode=False):
        """
        ...
        for pkg_name, loc in action.items():
            pkgs_to_process = [loc] if isinstance(loc, str) else loc
            pkgs_filtered = [
                pkg_key
                for pkg_key in pkgs_to_process
                if self._package_filter(pkg_key, strict_mode)
            ]
            if pkgs_filtered:
    ...
    def _package_filter(self, pkg, strict_mode):
        if not pkg:
            log.warn("RPM specified with no location")
            return False

        if self._is_installed(pkg, strict_mode):
    ...
    def order_versions(cls, pkg_vers):
        return sorted(pkg_vers, key=cmp_to_key(cls.compare_rpm_versions))
    ...
    def compare_rpm_versions(cls, first_pkg, second_pkg):
        ...
        for i in range(2):
            ...
            for j in range(min(len(first_chars), len(second_chars))):
                ...
                if c1.isdigit():
                    if c2.isdigit():
                        ...
                        int_cmp = (int(c1) > int(c2)) - (int(c1) < int(c2))
                        if int_cmp:
                            return int_cmp
                    else:
            ...
            second_chars_len = len(second_chars)
            len_cmp = (first_chars_len > second_chars_len) - (first_chars_len < second_chars_len)
            if len_cmp:
                return len_cmp
    ...
    def _is_installed(self, pkg, strict_mode):
        pkg_with_version = RpmTool.get_package_version(pkg)

        if not pkg_with_version or not pkg_with_version[0]:
            # In strict mode False is returned so that a retry can happen, otherwise just return True
